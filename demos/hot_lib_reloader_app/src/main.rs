...
//! # State persistence
//!
//! Same story as the other two host-resident drafts: a `lib` reload never
//! touches `count`/`ticks`/`last_key`, so the only state loss possible is a
//! full restart of `host` itself (needed when `src/main.rs` changes, since
//! `host` isn't hot-swappable — only `lib` is). `--persist-state` opts into
//! surviving that: a fixed-path JSON snapshot written after every tick and
//! key press, read back on startup if present.
//!
...
/// previous one wrote.
fn state_snapshot_path() -> PathBuf { env::temp_dir().join("termoxide-hot-lib-reloader-app.state.json") }
...
fn main() -> Result<()> {
    ...
    if persist_state {
        if let Some((saved_count, saved_ticks, saved_last_key)) = read_state_snapshot(&snapshot_path) {
            ...
            eprintln!("[hot_lib_reloader_app] restored state from {}", snapshot_path.display());
        }
        eprintln!("[hot_lib_reloader_app] state persistence on ({})", snapshot_path.display());
    }
