...
//!
//! # State persistence means something different here
//!
//! In the process-supervisor draft, "reload" and "the thing that loses
//! state" are the same event, so persistence was about surviving *that*.
//! Here they're not: a `logic` reload never touches `count`/`ticks`/
//! `last_key` at all — they're never at risk. The only way this draft loses
//! state is a full restart of `host` itself, which is needed only when
//! `host/src/main.rs` changes (it isn't hot-swappable — see the module
//! docs above). `--persist-state` opts into surviving *that*: `host` writes
//! its current state to a fixed path after every tick and key press, and
//! reads it back on startup if present. Fixed rather than pid-keyed,
//! deliberately — the whole point is finding the file across a pid change.
...
// ─────────────────────────────────────────────────────────────────────────
//  State persistence (opt-in, survives a `host` restart — see module docs)
// ─────────────────────────────────────────────────────────────────────────

/// Fixed path so a freshly started `host` (a new pid) can find what the
/// previous one wrote. One file for the whole machine: fine for a draft
/// with a single demo app, but the first thing a real version would need
/// to fix if two `dylib_app` instances ever ran at once.
fn state_snapshot_path() -> PathBuf { env::temp_dir().join("termoxide-dylib-app.state.json") }

fn write_state_snapshot(path: &Path, count: u32, ticks: u64, last_key: &str) {
    let value = serde_json::json!({ "count": count, "ticks": ticks, "last_key": last_key });
...
fn read_state_snapshot(path: &Path) -> Option<(u32, u64, String)> {
    let text = fs::read_to_string(path).ok()?;
    let value: serde_json::Value = serde_json::from_str(&text).ok()?;
    Some((
        u32::try_from(value.get("count")?.as_u64()?).ok()?,
        value.get("ticks")?.as_u64()?,
        value.get("last_key")?.as_str()?.to_string(),
    ))
}
...
fn main() -> Result<()> {
    let persist_state = env::args().any(|arg| arg == "--persist-state");
    ...
    let snapshot_path = state_snapshot_path();
    if persist_state {
        if let Some((saved_count, saved_ticks, saved_last_key)) = read_state_snapshot(&snapshot_path) {
            count.set(saved_count);
            ticks.set(saved_ticks);
            last_key.set(saved_last_key);
            eprintln!("[dylib_app host] restored state from {}", snapshot_path.display());
        }
        eprintln!("[dylib_app host] state persistence on ({})", snapshot_path.display());
    }
    ...
    while !quit_requested {
        ...
        for event in events.poll_events() {
            match event {
                ...
                Event::KeyPress(key) => {
                    ...
                    if persist_state {
                        write_state_snapshot(&snapshot_path, count.get_untracked(), ticks.get_untracked(), &last_key.get_untracked());
                    }
                },
            }
        }
        ...
        if last_tick.elapsed() >= TICK_INTERVAL {
            ...
            if persist_state {
                write_state_snapshot(&snapshot_path, count.get_untracked(), new_ticks, &last_key.get_untracked());
            }
        }
