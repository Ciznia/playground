...
//! # State persistence
//!
//! Same situation as the dylib draft: a `logic` reload never touches
//! `count`/`ticks`, so the only state loss this process can suffer is a
//! full restart of `host` itself (needed when `host/src/main.rs` changes,
//! since `host` isn't hot-swappable). `--persist-state` opts into
//! surviving that, the same way: a fixed-path JSON snapshot written after
//! every tick and key press, read back on startup if present.

use std::{env, fs, io::stdout, path::{Path, PathBuf}, process::Command, sync::mpsc, time::{Duration, Instant}};
...
/// previous one wrote.
fn state_snapshot_path() -> PathBuf { env::temp_dir().join("termoxide-wasm-app.state.json") }

fn write_state_snapshot(path: &Path, count: u32, ticks: u64) {
    let value = serde_json::json!({ "count": count, "ticks": ticks });
    let _ = fs::write(path, value.to_string());
}

fn read_state_snapshot(path: &Path) -> Option<(u32, u64)> {
    let text = fs::read_to_string(path).ok()?;
    let value: serde_json::Value = serde_json::from_str(&text).ok()?;
    Some((u32::try_from(value.get("count")?.as_u64()?).ok()?, value.get("ticks")?.as_u64()?))
}
...
fn main() -> Result<()> {
    ...
    if persist_state {
        if let Some((saved_count, saved_ticks)) = read_state_snapshot(&snapshot_path) {
            count.set(saved_count);
            ticks.set(saved_ticks);
            eprintln!("[wasm_app host] restored state from {}", snapshot_path.display());
        }
        eprintln!("[wasm_app host] state persistence on ({})", snapshot_path.display());
    }
    ...
    while !quit_requested {
        ...
        for event in events.poll_events() {
            if let Event::KeyPress(key) = event {
                ...
                quit_requested = quit;
                if persist_state {
                    write_state_snapshot(&snapshot_path, count.get_untracked(), ticks.get_untracked());
                }
            }
        }
        ...
        if last_tick.elapsed() >= TICK_INTERVAL {
            ...
            if persist_state {
                write_state_snapshot(&snapshot_path, count.get_untracked(), new_ticks as u64);
            }
        }
