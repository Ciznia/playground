//! Dev-time supervisor for hot reload — `watchexec`-as-library variant.
//!
//! Identical to the `hot_reload_process_supervisor` draft in every respect
//! except one: file-watching is delegated to the [`watchexec`] crate (the
//! library `cargo-watch`/`watchexec-cli` are themselves built on) instead
//! of a hand-rolled `notify` + manual debounce loop. Everything else —
//! the blue/green slot rebuild, the sentinel-based graceful shutdown, the
//! Ctrl-C handling — is the exact same code as that draft; only
//! [`spawn_watchexec_watcher`] differs from that draft's inline `notify`
//! setup in `main`.
//!
//! # Why a whole extra thread and a Tokio runtime
//!
//! `watchexec` is async, built on Tokio, with no synchronous API. The rest
//! of this supervisor is deliberately synchronous (same as the
//! `notify`-based draft — a dev tool has no real need for an async
//! runtime otherwise). Pulling in Tokio for the *whole* binary just to
//! satisfy `watchexec` felt like the wrong trade for what should be a
//! small, swappable piece, so instead a dedicated single-thread runtime
//! runs on its own background OS thread, and forwards change
//! notifications to the same synchronous `mpsc` channel the main loop
//! already reads via [`wait_for_change`] — same consumer-side code as the
//! `notify` draft, different producer.
//!
//! `termoxide-watch-wx` never touches a running app's terminal or process
...
//!
//! No application state survives a reload: the new process starts from
//! scratch, same as a normal `cargo run`. See the
//! `hot_reload_process_supervisor_persist` draft worktree for the opt-in
//! state-persistence variant built on top of this one.
//!
//! ## Ctrl-C
//!
...
//!
//! ```text
//! cargo run -p termoxide_watch_wx -- --manifest-path demos/external_app/Cargo.toml
//! ```
...
use watchexec::{Watchexec, action::ActionHandler};
...
fn parse_args() -> Result<Args> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            other => {
                bail!(
                    "unrecognized argument: {other}\n\nUsage: termoxide-watch-wx --manifest-path <path/to/Cargo.toml> \
                     [--bin <name>]"
                )
            },
        }
    ...
    Ok(Args {
        manifest_path: manifest_path.context(
            "missing required --manifest-path <path/to/Cargo.toml>\n\nUsage: termoxide-watch-wx --manifest-path \
             <path/to/Cargo.toml> [--bin <name>]",
        )?,
        bin,
    })
}
...
fn build(manifest_path: &Path, bin_name: &str) -> Result<Option<PathBuf>> {
    ...
    if !output.status.success() {
        eprintln!("[termoxide-watch-wx] build failed; leaving the running app as-is and watching for the next change");
        return Ok(None);
    }
...
fn spawn(executable: &Path, crate_dir: &Path) -> Result<RunningApp> {
...
fn stop(mut app: RunningApp) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            eprintln!("[termoxide-watch-wx] app did not exit within {SHUTDOWN_TIMEOUT:?}; killing it");
...
fn install_interrupt_handler() -> Result<Arc<AtomicBool>> {
    ...
    ctrlc::set_handler(move || {
        if flag.swap(true, Ordering::SeqCst) {
            eprintln!("[termoxide-watch-wx] second Ctrl-C, forcing immediate exit");
            std::process::exit(130);
        }
        eprintln!("[termoxide-watch-wx] Ctrl-C received, shutting down (press again to force)");
    })
    .context("failed to install Ctrl-C handler")?;
    Ok(interrupted)
}

/// Wait for either the next (already-debounced) change notification or
/// Ctrl-C.
///
...
/// thread exited).
///
/// Unlike the `notify`-based draft's version of this function, there's no
/// follow-up drain loop needed here: `watchexec`'s own `Config::throttle`
/// (set in [`spawn_watchexec_watcher`]) already debounces on the trailing
/// edge, so every message on `rx` is already a settled change, not a raw
/// per-file-write event.
fn wait_for_change(rx: &mpsc::Receiver<()>, interrupted: &AtomicBool) -> bool {
    loop {
...
/// Watch `paths` for changes using `watchexec`, forwarding a debounced
/// notification for each settled change to the returned channel.
///
/// Runs `watchexec`'s (Tokio-based) event loop on its own background OS
/// thread with a dedicated single-thread runtime — see the module docs for
/// why that's preferable to making the whole supervisor async. The thread
/// is a daemon: it's never joined, and simply ends when the process does.
fn spawn_watchexec_watcher(paths: Vec<PathBuf>) -> mpsc::Receiver<()> {
    let (tx, rx) = mpsc::channel();
    // `mpsc::Sender` is `Send` but not `Sync`, and `watchexec`'s action
    // handler must be both (it may be invoked from any of its worker
    // tasks) — the `Mutex` is only ever used single-threaded in practice,
    // just to satisfy that bound.
    let tx = Arc::new(Mutex::new(tx));

    std::thread::spawn(move || {
        let Ok(runtime) = tokio::runtime::Builder::new_current_thread().enable_all().build() else {
            eprintln!("[termoxide-watch-wx] failed to start watchexec's Tokio runtime");
            return;
        };

        if let Err(e) = runtime.block_on(run_watchexec(paths, tx)) {
            eprintln!("[termoxide-watch-wx] watchexec stopped: {e:#}");
        }
    });
...
async fn run_watchexec(paths: Vec<PathBuf>, tx: Arc<Mutex<mpsc::Sender<()>>>) -> Result<()> {
    let wx = Watchexec::new(move |action: ActionHandler| {
        if !action.events.is_empty()
            && let Ok(tx) = tx.lock()
        {
            let _ = tx.send(());
        }
        // We never call `action.quit()`: Ctrl-C is handled by this
        // process's own `ctrlc` handler (see `install_interrupt_handler`),
        // not by `watchexec`'s. Letting `watchexec` keep running is
        // harmless — its thread dies with the process either way.
        action
    })
    .map_err(|e| anyhow::anyhow!("failed to start watchexec: {e}"))?;

    wx.config.pathset(paths);
    wx.config.throttle(DEBOUNCE);

    wx.main()
        .await
        .context("watchexec's task panicked")?
        .map_err(|e| anyhow::anyhow!("watchexec error: {e}"))?;
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "[termoxide-watch-wx] watching {} (bin: {})",
    ...
    let slot_path = build_into_slot(&args.manifest_path, &target.bin_name, &target.run_dir, slot)?
        .context("initial build failed; fix the error above and re-run termoxide-watch-wx")?;
    let mut running = spawn(&slot_path, &target.crate_dir)?;

    let src_dir = target.crate_dir.join("src");
    let rx = spawn_watchexec_watcher(vec![src_dir, args.manifest_path.clone()]);

    while wait_for_change(&rx, &interrupted) {
        eprintln!("[termoxide-watch-wx] change detected, rebuilding...");
        ...
        if let Some(slot_path) = build_into_slot(&args.manifest_path, &target.bin_name, &target.run_dir, next_slot)? {
            stop(running)?;
            running = spawn(&slot_path, &target.crate_dir)?;
            slot = next_slot;
            eprintln!("[termoxide-watch-wx] reloaded");
        }
        // `None` means `build` already reported the failure; `running` on
        // the current slot is untouched, so the app never went down.
    }

    eprintln!("[termoxide-watch-wx] stopping the running app and exiting");
    stop(running)?;
    Ok(())
}
