...
//!
//! Because host already only ever holds the app's state as an opaque,
//! already-`postcard`-encoded `Vec<u8>` (see above), persisting it across
//! a restart needs no new encoding step at all: [`run_persistent`] just
//! writes that same byte blob to a fixed path after every state change,
//! and reads it back as the starting `state_bytes` on the next run. No
//! `Serialize`/`Deserialize` bound, no derive, nothing for the app to opt
//! into beyond calling `run_persistent` instead of `run` — a strictly
//! smaller app-side cost than the hot-lib-reloader form of this crate,
//! where a live, in-process `S` value still had to be encoded specially
...
pub fn run(config: WasmConfig) -> Result<()> { drive(config, Vec::new(), |_| {}) }
...
/// state change. See the module docs for why no encoding step (and no
/// trait bound on an app-specific type) is needed to do this here.
pub fn run_persistent(config: WasmConfig, snapshot_path: PathBuf) -> Result<()> {
    ...
    drive(config, initial, |bytes| {
        // Best-effort: a failed write here shouldn't take down a running
        // app over a dev-time convenience feature.
        let _ = std::fs::write(&snapshot_path, bytes);
    })
}

/// Shared loop body for [`run`] and [`run_persistent`]: rebuild watcher,
/// wasmtime setup, terminal setup, event/tick/render loop, teardown.
/// `after_state_change` is called with the latest opaque state bytes
/// after every event and tick — [`run`] gives it a no-op,
/// [`run_persistent`] gives it the snapshot writer.
fn drive(config: WasmConfig, initial_state: Vec<u8>, mut after_state_change: impl FnMut(&[u8])) -> Result<()> {
